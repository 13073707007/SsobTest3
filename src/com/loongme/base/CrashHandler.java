...
import com.loongme.util.DateUtil;
	...
	public void uncaughtException(Thread thread, Throwable ex) {
		if (!handleException(ex) && mDefaultHandler != null) {
			...
			try {
				...
				e.printStackTrace();
				Log.e(Configuration.TAG, "error : ", e);
			}
			// 退出程序
		...
		try {
			String fileName = "log-"
					+ DateUtil.getTimeStamp(new Date(),
							DateUtil.YYYY_MM_DD_HH_MM_SS_SHORT) + ".txt";
			String path = "";
			// sd卡
			if (Environment.getExternalStorageState().equals(
					Environment.MEDIA_MOUNTED)) {
				path = Configuration.ERROR_LOG_DIR;
			} else {
				// 内存
				path = mContext.getFilesDir().getAbsolutePath()
						+ File.separator + Configuration.ERROR_LOG_DIR;
			}
			...
			Log.e(Configuration.TAG, "an error occured while writing file...",
					e);
		}
		return null;
	}
