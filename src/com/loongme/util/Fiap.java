...
public class Fiap {
		...
		public void handleMessage(Message msg) {
			...
			try {
				...
				if (bRet) {
					// 显示“正在支付”进度条
					closeProgress();
					mProgress = BaseHelper.showProgress(mActivity, null,
							"正在支付", false, true);
				}
			} catch (Exception ex) {
				ex.printStackTrace();
	...
	String getOrderInfo(double position) {
		...
		strOrderInfo += "subject=" + "猫币";
		strOrderInfo += "&";
		strOrderInfo += "body=" + "购买猫币";
		strOrderInfo += "&";
		strOrderInfo += "total_fee=" + "" + position + "";
		strOrderInfo += "&";
		strOrderInfo += "notify_url=" + ""
				+ "http://notify.java.jpxx.org/index.jsp" + "";
		Log.i(TAG, "获取订单：" + strOrderInfo);
		return strOrderInfo;
	}
	...
	public static class BaseHelper {
		...
		public static ProgressDialog showProgress(Context context,
			...
			dialog.setOnCancelListener(new Fiap.AlixOnCancelListener(
					(Activity) context));
			dialog.show();
			return dialog;
		}
